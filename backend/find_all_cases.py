#!/usr/bin/env python3
"""
Script untuk mencari semua case yang mungkin sesuai dengan parameter user
"""

import pandas as pd
from services.cost_analysis import run_economic_analysis

def find_all_possible_cases():
    """
    Mencari semua case yang mungkin sesuai dengan parameter user dengan toleransi yang lebih besar
    """
    ...
    try:
        ...
        print("\n=== MENCARI CASE DENGAN TOLERANSI LEBIH BESAR ===")
        
        for case in user_cases:
            print(f"\n=== {case['name']} ===")
            params = case['params']
            
            # Create filter conditions with larger tolerance
            conditions = []
            
            # Temperature (±20°C tolerance)
            if 'temp' in params:
                temp_condition = (
                    (syn1['Temperature (oC)'] >= params['temp'] - 20) & 
                    (syn1['Temperature (oC)'] <= params['temp'] + 20)
                )
                conditions.append(temp_condition)
                print(f"Looking for Temperature: {params['temp']}°C (±20°C)")
            ...
            # Metal (contains all keywords)
            if 'metal_contains' in params:
                metal_condition = True
                for keyword in params['metal_contains']:
                    keyword_condition = syn1['Metal_Name'].str.contains(keyword, case=False, na=False)
                    metal_condition = metal_condition & keyword_condition
                
                conditions.append(metal_condition)
                print(f"Looking for Metal containing: {params['metal_contains']}")
                ...
                if len(matching_rows) > 0:
                    ...
                    for idx, row in matching_rows.iterrows():
                        ...
                        try:
                            ...
                            mof_diff = abs(actual_mof - expected_mof)
                            storage_diff = abs(actual_storage - expected_storage)
                            total_diff = mof_diff + storage_diff
                            
                            print(f"    CALCULATION RESULT:")
                            print(f"      Expected MOF Cost: {expected_mof}")
                            print(f"      Actual MOF Cost:   {actual_mof:.4f}")
                            print(f"      Difference:        {mof_diff:.4f}")
                            print(f"      Expected Storage:  {expected_storage}")
                            print(f"      Actual Storage:    {actual_storage:.4f}")
                            print(f"      Difference:        {storage_diff:.4f}")
                    ...
                    if best_match:
                        print(f"\n  *** BEST MATCH for {case['name']} ***")
                        print(f"  Row {best_match['row']} - {best_match['mof_name']}")
                        print(f"  MOF Cost: {best_match['actual_mof']:.4f} (expected: {case['expected']['mof_cost']})")
                        print(f"  Storage Cost: {best_match['actual_storage']:.4f} (expected: {case['expected']['storage_cost']})")
                        print(f"  Total Difference: {best_match['total_diff']:.4f}")
